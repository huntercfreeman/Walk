...
public ref partial struct CSharpParserState
{
    ...
    public TypeReferenceValue MostRecentLeftHandSideAssignmentExpressionTypeClauseNode { get; set; } = CSharpFacts.Types.Void.ToTypeReference();
        ...
        bool shouldCreateSymbol = true)
    {
                ...
                out var variableDeclarationNode))
        {
            ...
            typeReference = variableReferenceMetadata.TypeReference;
        ...
        out SyntaxNodeValue typeDefinitionNode)
    {
        ...
        if (typeDefinitionNode.IsDefault())
        {
            if (definitionScopeSubIndex == 0)
            {
                ...
                {
                    if (Binder.CSharpCompilerService.SafeCompareTextSpans(
                            referenceResourceUri.Value, referenceTextSpan, externalDefinitionNode.ResourceUri.Value, externalDefinitionNode.IdentifierToken.TextSpan))
                    {
        ...
        bool isRecursive = false)
    {
        ...
        for (int i = declarationCompilationUnit.NodeOffset; i < declarationCompilationUnit.NodeOffset + declarationCompilationUnit.NodeLength; i++)
        {
            ...
            {
                if (Binder.CSharpCompilerService.SafeCompareTextSpans(
                        referenceResourceUri.Value, referenceTextSpan, declarationResourceUri.Value, node.IdentifierToken.TextSpan))
                {
        ...
        if (variableDeclarationNode.IsDefault())
        {
            ...
            if (!isRecursive && scope.OwnerSyntaxKind == SyntaxKind.TypeDefinitionNode)
            {
                ...
                if (!typeDefinitionMetadata.InheritedTypeReference.IsDefault())
                {
                    ...
                    if (!typeDefinitionScopeIndexKey.IsDefault())
                    {
                        ...
                        {
                            if (!inheritedTypeDefinitionNode.IsDefault())
                            {
