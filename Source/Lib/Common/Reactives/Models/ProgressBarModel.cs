...
namespace Walk.Common.RazorLib.Reactives.Models;
...
public class ProgressBarModel : IDisposable
{
	...
	{
		if (IsDisposed)
			throw new WalkCommonException($"The {nameof(ProgressBarModel)} has {nameof(IsDisposed)} set to true, therefore cannot be set anymore.");
